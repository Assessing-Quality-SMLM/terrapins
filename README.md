...
### Drift
This report compares two different FRC curves generated by different ways to split localisation lists to assess if the sample is drifting. The score is generated by comparing their resolution.

The main result looks like this

This plot shows the two FRC curves. The further apart the two curves are the more drift is present.

The different splits of the data are half (by number of frames) vs section splitting. 
...
The blinking report compares 2 different techniques for the splitting the data, section vs interleaved, and compares their ratio. Section splitting should 

The plot shows both FRC curves for comparison.
...
The plot shows the FRC curve plotted with where we suggest these tolerances lie.

### Bias / HAWKMAN

The bias report attempt to establish where and by how much the fitter has introduced bias. The image show in this plot attempts to colour code structure by the severity of the bias detected. When this score approaches 1 there is no bias left and the level that this it at determines the resolution (level * super resolution pixel size).
