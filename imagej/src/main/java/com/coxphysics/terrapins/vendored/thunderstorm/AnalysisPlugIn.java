...
import static com.coxphysics.terrapins.vendored.thunderstorm.util.ImageMath.subtract;
import static com.coxphysics.terrapins.vendored.thunderstorm.util.ImageMath.add;
...
import com.coxphysics.terrapins.vendored.thunderstorm.UI.StoppedDueToErrorException;
...
import com.coxphysics.terrapins.vendored.thunderstorm.results.MeasurementProtocol;
...
public final class AnalysisPlugIn implements ExtendedPlugInFilter {
    ...
    synchronized public void showResults() {
        ...
        // Rendering (renderLater, once per processed frame with detections) runs on its own
        // background thread and does not otherwise block the caller. Without waiting here, "Run
        // analysis" can return -- and a macro can proceed to read/save the rendered image -- before
        // that background thread has caught up on its backlog, yielding a partially-accumulated
        // image with a display range set from that same incomplete state (or, in the worst case,
        // a still-blank image).
        renderingQueue.waitForFinished();
    }
}
