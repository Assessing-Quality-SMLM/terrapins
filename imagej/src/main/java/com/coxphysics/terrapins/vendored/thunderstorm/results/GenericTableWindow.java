...
public class GenericTableWindow {
    ...
    public GenericTableWindow(String frameTitle) {
        // Defensive: ij.ImageJ's own quit-handling ("Quit" thread, spawned from its WindowListener)
        // ends with System.exit(0) only when this flag is set -- which ij.ImageJ.main() does for a
        // real standalone launch, but a plain `new ImageJ()` (as any embedding host does) leaves it
        // false. Setting it explicitly here means that even if something disposes the main ImageJ
        // window later (e.g. a window manager deciding it's unresponsive while this class does the
        // synchronous EDT work below, and sending it a close request), that can never take the whole
        // host application down with it -- only ImageJ's own window/state gets torn down.
