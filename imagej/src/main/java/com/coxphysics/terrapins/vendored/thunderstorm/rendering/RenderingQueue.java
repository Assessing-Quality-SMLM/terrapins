package com.coxphysics.terrapins.vendored.thunderstorm.rendering;
...
public class RenderingQueue {
    ...
    /**
     * Blocks until every task submitted so far (renderLater/repaintLater/invokeLater/resetLater)
     * has finished running. Since all tasks run on a single thread in submission order, this is
     * done by submitting one more no-op task and waiting for it to run -- by the time it does,
     * everything submitted before it is guaranteed to have already completed.
     *
     * Without this, a caller that queues rendering work and then immediately reads/saves the
     * rendered image (e.g. a macro doing `run("Run analysis", ...); saveAs(...)`) can race the
     * background rendering thread: the image may still be missing some, or all, of the
     * localizations, and its display range (set by the periodic repaint task) may reflect that
     * incomplete state rather than the final one.
     */
