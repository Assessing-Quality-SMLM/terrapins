...
import com.coxphysics.terrapins.vendored.thunderstorm.CameraSetupPlugIn;
...
public class AnalysisOptionsDialog extends JDialog implements ActionListener {
    ...
    public AnalysisOptionsDialog(final ImagePlus imp, String title,
        ...
        //
        this.activeFilterIndex = Integer.parseInt(Prefs.get("terrapins.thunderstorm.filters.index", "0"));
        this.activeDetectorIndex = Integer.parseInt(Prefs.get("terrapins.thunderstorm.detectors.index", "0"));
        this.activeEstimatorIndex = Integer.parseInt(Prefs.get("terrapins.thunderstorm.estimators.index", "0"));
    ...
    static void saveSelectedModuleIndexesToPrefs(int filterIndex, int detectorIndex, int estimatorIndex, int rendererIndex) {
        if(filterIndex >= 0) {
            Prefs.set("terrapins.thunderstorm.filters.index", filterIndex);
        }
        if(detectorIndex >= 0) {
            Prefs.set("terrapins.thunderstorm.detectors.index", detectorIndex);
        }
        if(estimatorIndex >= 0) {
            Prefs.set("terrapins.thunderstorm.estimators.index", estimatorIndex);
        }
        if(rendererIndex >= 0) {
            Prefs.set("terrapins.thunderstorm.rendering.index", rendererIndex);
        }
    }
}
