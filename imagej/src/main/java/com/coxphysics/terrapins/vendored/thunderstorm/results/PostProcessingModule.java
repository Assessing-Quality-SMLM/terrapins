...
public abstract class PostProcessingModule {
    ...
    public void run() {
        try {
            // getUIPanel() lazily builds the panel and its fields (applyButton, balloon tips, ...)
            // on first call and caches it -- ResultsTableWindow no longer builds every module's panel
            // up front, so runImpl() (which references those fields directly, e.g. applyButton) needs
            // this guaranteed to have run at least once before it does, however this module was invoked
            // (macro, or the "Apply" button, which itself lives inside the panel getUIPanel() builds).
            getUIPanel();
            GUI.closeBalloonTip();
