...
import com.coxphysics.terrapins.vendored.thunderstorm.UI.*;
...
import com.coxphysics.terrapins.vendored.thunderstorm.estimators.ui.IBiplaneEstimatorUI;
...
public final class BiplaneAnalysisPlugIn implements PlugIn {
    ...
    synchronized private void showResults() {
        ...
        */
        // Without this, a macro doing `run("Run analysis", ...); saveAs(...);` can race
        // renderLater()'s background thread and save a partially-rendered (or still blank) image --
        // see AnalysisPlugIn#showResults, which has the same fix for the same reason.
